		...
		axios
			...
			.then((resp) => {
				dispatch({
					...
					value: [
						...ctx.data,
						...resp.data.data.filter((d: Genre) => !ctx.data.find((dd) => dd.id === d.id))
					]
				});
				...
				{ctx.queries.sort === 'NAME'
					? letterGroup.map(
							(l) =>
								ctx.data.filter(
									(d) =>
										(!l.match(/[a-z]/i) && !d.name.charAt(0).toLowerCase().match(/[a-z]/i)) ||
										d.name.charAt(0).toLowerCase() === l
								).length > 0 && (
									<Fragment key={l}>
										<div className="text-primary col-span-2 text-2xl font-bold md:col-span-3 lg:col-span-4 xl:col-span-5 2xl:col-span-6">
											# {l.toUpperCase()}
										</div>
										{ctx.data
											.filter(
											...
											.map((d) => (
												<Link
													key={d.id}
													title={d.name}
													className="hover:border-primary flex flex-nowrap items-center justify-between gap-1 rounded border border-neutral-800 p-2 font-bold hover:opacity-100"
													href={`/studios/${d.id}/${toURL(d.name)}`}
												>
