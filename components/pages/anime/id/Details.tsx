...
export default function Details({ anime }: { anime: Anime }) {
	return (
		<div className="grid grid-cols-2 gap-4 text-center font-bold sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6">
			...
			/>
			<Stats title="average score" top="Score" value={anime.mean ? anime.mean.toFixed(2) : 'N/A'} />
			<Stats title="user count who give score" top="Voter" value={anime.voter.toLocaleString()} />
			<Stats
				title="popularity rank"
				top="Popularity"
				value={anime.popularity ? '#' + anime.popularity.toLocaleString() : 'N/A'}
			/>
			<Stats
			...
			<Stats title="anime source" top="Source" value={AnimeSourceToStr(anime.source)} />
			<Stats title="airing status" top="Status" value={AnimeStatusToStr(anime.status)} />
			{anime.season?.season && (
				<Stats
					title="season"
					top="Season"
			...
			<Stats
				title="episode"
				top="Episode"
				value={anime.episode.count !== 0 ? anime.episode.count.toLocaleString() : '-'}
				bottom={
					anime.episode.duration === 0
						? undefined
						: `${parseInt((anime.episode.duration / 60).toFixed(0)).toLocaleString()} min`
				}
			/>
			<Stats
...
const Stats = ({
	...
	return (
		<motion.div
			...
			whileInView="animate"
		>
			<div className="text-primary pointer-events-none text-sm">{top}</div>
