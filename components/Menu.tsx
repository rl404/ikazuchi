...
export default function Menu() {
	...
	const onClick = () => {
		if (!open) setBgColor(shuffle(BackgroundColors)[0]);
		toggleOpen();
	};
	...
	return (
		<motion.nav initial={false} animate={open ? 'open' : 'closed'} custom={height}>
			<motion.div
				variants={menuVariants}
				className={`py-cinematic-bar fixed top-0 bottom-0 left-0 h-full w-full overflow-scroll ${bgColor}`}
			>
				<div className="container mx-auto flex flex-col items-center justify-center py-4">
					<div className="flex flex-col items-center justify-center">
						<Link
							href="/"
							className="font-mincho mb-2 border-2 border-transparent px-4 py-2 text-4xl font-bold tracking-widest hover:border-white hover:text-white hover:opacity-100"
						>
							イカヅチ
						</Link>
					...
					<div className="gap-cinematic-bar grid grid-cols-6 text-center">
						<div className="col-span-6 flex flex-col gap-2 sm:col-span-3 md:col-span-2">
							<div className="mb-4 flex flex-col">
								<Link href="/studios" className="font-mincho text-2xl font-bold">
									スタジオ
								</Link>
						...
						<div className="col-span-6 flex flex-col gap-2 sm:col-span-3 md:col-span-2">
							...
							<Link href="/anime?status=NOT_YET&sort=POPULARITY">Top Upcoming</Link>
							<Link
...
const MenuIcon = ({ toggle }: { toggle: () => void }) => {
	return (
		<button onClick={toggle} className="fixed top-4 left-4 z-10 flex items-center">
