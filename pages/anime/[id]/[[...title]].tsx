...
export default function AnimePage(data: InferGetStaticPropsType<typeof getStaticProps>) {
	if (!data.data) {
		return (
			<div className="flex h-full items-center justify-center">
				<SpinnerIcon className="fill-primary h-10 w-10 animate-spin text-neutral-600" />
			</div>
		);
	}
	...
	return (
		<>
			<Head
				title={anime.title}
				description={Ellipsis(anime.synopsis, 150) || 'No anime synopsis information.'}
				image={anime.picture}
			/>
...
export const getStaticProps: GetStaticProps<Data> = async ({ params }) => {
	const res = await fetch(`${process.env.API_HOST_AKATSUKI}/anime/${params?.id}`);
