...
export default function StudioPage(data: InferGetStaticPropsType<typeof getStaticProps>) {
	if (!data.data) {
		return (
	...
	return (
		<>
			<Head title={studio.name} description={`${studio.name} anime studio.`} image="" />
...
export const getStaticProps: GetStaticProps<Data> = async ({ params }) => {
	const res = await fetch(`${process.env.API_HOST_AKATSUKI}/studios/${params?.id}`);
