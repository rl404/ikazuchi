...
export default function GenrePage(data: InferGetStaticPropsType<typeof getStaticProps>) {
	if (!data.data) {
		return (
	...
	return (
		<>
			<Head title={genre.name} description={`${genre.name} anime genre.`} image="" />
...
export const getStaticProps: GetStaticProps<Data> = async ({ params }) => {
	const res = await fetch(`${process.env.API_HOST_AKATSUKI}/genres/${params?.id}`);
